...
class OvernightSubmissionsProcessingServiceTest extends FunSuite {
  ...
  test("OvernightSubmissionsProcessingService.updateOvernightData") {
    // arrange
    // run init data load
    val directory = new Directory(new File(pathToDestination))
    directory.deleteRecursively()
    var pathToSource = getClass.getResource("/overnight-submissions/overnight_submissions_new_business_sub_id_1001_cnt_10_10d_from_now_1.csv")
    var isInitialRun: Boolean = true
    var args: Array[String] = Array(pathToSource.getPath, pathToDestination, isInitialRun.toString, isLocalRun.toString)
    val processor = new OvernightSubmissionsProcessingService()
    processor.processOvernightData(args)

    // act (process update)
    pathToSource = getClass.getResource("/overnight-submissions/overnight_submissions_new_business_sub_id_1001_cnt_10_10d_from_now_2_changed_sub_1001_status.csv")
    isInitialRun = false
    args = Array(pathToSource.getPath, pathToDestination, isInitialRun.toString, isLocalRun.toString)
    processor.processOvernightData(args)
    ...
    try {
      ...
      println(s"Sub status = $status, Underwriter = $underwriter")
      assert(status === "wip")
      assert(underwriter === "Cool Underwriter!")
    }
    finally {
      if (spark != null) {
        spark.stop()
